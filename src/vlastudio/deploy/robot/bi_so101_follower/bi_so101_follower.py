...
from .config import BiSO101FollowerConfig
...
class BiSO101Follower(Robot):
    ...
    def __init__(self, config: BiSO101FollowerConfig):
        ...
        # Use custom arm IDs if provided, otherwise default to "{id}_left" / "{id}_right"
        left_id = config.left_arm_id if config.left_arm_id else (f"{config.id}_left" if config.id else None)
        right_id = config.right_arm_id if config.right_arm_id else (f"{config.id}_right" if config.id else None)
