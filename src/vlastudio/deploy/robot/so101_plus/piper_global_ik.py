"""
Piper-style global IK solver for SO101 Plus (local copy, not shared with Alicia-D).

Formulation:
- optimize joint positions with casadi+ipopt when available, else scipy L-BFGS-B
- minimize weighted SE(3) pose error
- add per-joint regularization (favor distal wrist_yaw / wrist_roll)
- warm start from the previous solution

EE frame is configurable via ``end_frame`` + optional ``ee_offset_matrix``.
Default lock joint is ``joint7`` (gripper).
"""
...
class PiperStyleGlobalIKSolver:
    """Global IK for SO101 Plus flange teleop (gripper_body)."""

    def __init__(
        self,
        urdf_path: str,
        end_frame: str = "gripper_body",
        ee_offset_matrix: Optional[np.ndarray] = None,
        lock_joint_names: Sequence[str] = ("joint7",),
        ...
        # Per-joint scales on the regularization term (length = reduced nq).
        # Larger = prefer NOT moving that joint. Keep wrist_yaw/roll small so
        # orientation uses those servos instead of shoulder_pan.
        ...
        self._joint_reg_weights_arg = (
            None
            if joint_regularization_weights is None
            else np.asarray(joint_regularization_weights, dtype=np.float64).reshape(-1)
        )
        ...
        nq = int(self.reduced_model.nq)
        if self._joint_reg_weights_arg is None:
            self.joint_regularization_weights = np.ones(nq, dtype=np.float64)
        else:
            w = self._joint_reg_weights_arg
            if w.size != nq:
                raise ValueError(
                    f"joint_regularization_weights length {w.size} != reduced nq {nq}"
                )
            self.joint_regularization_weights = np.maximum(w.astype(np.float64), 1e-8)
        ...
        try:
            ...
            w_reg = casadi.DM(self.joint_regularization_weights.reshape(-1, 1))
            ...
            if self.regularize_to_warmstart:
                dq = self.var_q - self.param_q_reg
            else:
                dq = self.var_q
            regularization = casadi.sumsqr(casadi.sqrt(w_reg) * dq)
    ...
    def set_reference(self, q_ref: np.ndarray, pose_ref: Optional[np.ndarray] = None):
        """Compatibility hook for rel_ee anchor updates."""
    ...
    def _solve_impl(self, target_pose: np.ndarray, q_init: np.ndarray) -> Tuple[bool, np.ndarray]:
        ...
        def objective(q: np.ndarray) -> float:
            ...
            if self.regularize_to_warmstart:
                ...
                dq = q
            regularization = float(np.sum(self.joint_regularization_weights * (dq**2)))
