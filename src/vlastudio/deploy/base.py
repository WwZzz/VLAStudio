...
from vlastudio.deploy.shm_utils import SharedMemoryChannel, _fix_resource_tracker
...
def is_camera_config(config:dict) -> bool:
    ...
    """
    device_class = resolve(config['type'])
    from vlastudio.deploy.sensor.camera.opencv_camera import OpenCVCamera
...
def create_enrich_saved_frame_func(device_configs: list):
    ...
    """
    from vlastudio.deploy.utils import partition_visualizer_configs
