...
class CNNMLP(nn.Module):
    def __init__(self, backbones, state_dim, action_dim, camera_names, chunk_size=1):
        """ Initializes the model.
        Parameters:
            backbones: torch module of the backbone to be used. See backbone.py
            transformer: torch module of the transformer architecture. See transformer.py
        ...
        if backbones is not None:
            ...
            self.mlp = mlp(input_dim=mlp_in_dim, hidden_dim=1024, output_dim=self.action_dim*self.chunk_size, hidden_depth=2)
        else:
    ...
    def forward(self, qpos, image, env_state, actions=None):
        ...
        return a_hat.reshape(bs, self.chunk_size, self.action_dim)
