...
class BiSO101Dataset(EpisodicDataset):
    """
    ...
    def load_onestep_from_episode(self, dataset_path, start_ts=None):
        ...
        # Load images
        image_dict = dict(
            primary = cv2.resize(root[f'/observations/head_camera'][start_ts], self.image_size),
            left_wrist = cv2.resize(root[f'/observations/left_wrist_camera'][start_ts], self.image_size),
            right_wrist = cv2.resize(root[f'/observations/right_wrist_camera'][start_ts], self.image_size),
        )
    ...
    def load_feat_from_episode(self, dataset_path, feats=[]):
        ...
        with h5py.File(dataset_path, 'r') as root:
            ...
            if 'image_primary' in feats or 'image' in feats or len(feats) == 0:  # Load images
                img_bytes = root[f'/observations/head_camera'][()]
                image_dict.update(
                    dict(
